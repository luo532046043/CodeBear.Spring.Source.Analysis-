...
/**
 * PathMatchingResourcePatternResolver类实现了ResourcePatternResolver接口，它包含了对ResourceLoader接口的引用，
 * 在对继承自ResourceLoader接口的方法的实现会代理给该引用，同时在getResources()方法实现中，当找到一个匹配的资源location时，
 * 可以使用该引用解析成Resource实例。默认使用DefaultResourceLoader类，用户可以使用构造函数传入自定义的ResourceLoader。
 */
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	/**
	 * 对PathMatcher接口的引用，该接口基于路径字符串实现匹配处理，如判断一个路径字符串是否包含通配符（’*’、’?’），
	...
	// 需要注意的是，由于ClassLoader.getResources()方法存在的限制，当传入一个空字符串时，它只能从classpath的文件目录下查找，
	// 而不会从Jar文件的根目录下查找，因而对”classpath*:”前缀的资源来说，找不到Jar根路径下的资源。即如果我们有以下定义：”classpath*:*.xml”，
	// 如果只有在Jar文件的根目录下存在*.xml文件，那么这个pattern将返回空的Resource数组。
	// 解决方法是不要再Jar文件根目录中放文件，可以将这些文件放到Jar文件中的resources、config等目录下去。并且也不要在”classpath*:”之后加一些通配符，
	// 如”classpath*:**/*Enum.class”，至少在”classpath*:”后加入一个不存在通配符的路径名。

	// classpath*:表示查找classpath路径下的所有符合条件的资源，包含jar、zip等资源；
	// classpath:表示优先在项目的资源目录下查找，找不到才去jar、zip等资源中查找
	...
	public Resource[] getResources(String locationPattern) throws IOException {
		...
		if (locationPattern.startsWith(CLASSPATH_ALL_URL_PREFIX)) {
			...
			// 去除 "classpath*:"这个后，再判断路径是否包含通配符"*"或者"?"
			...
			// Generally only look for a pattern after a prefix here,      //通常只看这里的前缀后面查找模式
			...
			//路径不包含通配符
				...
				//不含通配符和不是以"classpath*:"开头的, 直接委托给相应的 ResourceLoader 来实现。
	...
	protected Set<Resource> doFindAllClassPathResources(String path) throws IOException {
		...
		// 获取如果在构造 PathMatchingResourcePatternResolver 实例的时候如果传入了 ClassLoader，
		// 则调用 ClassLoader#getResources() 方法，否则调用 ClassLoader#getSystemResources(path) 方法
	...
	protected Resource[] findPathMatchingResources(String locationPattern) throws IOException {
		// 确定根路径，例如classpath*:com/question/
		...
		for (Resource rootDirResource : rootDirResources) {
			...
			//资源类型(jar协议、zip协议、wsjar协议、vfszip协议、war协议)
			else if (ResourceUtils.isJarURL(rootDirUrl) || isJarResource(rootDirResource)) {
				//从资源类型(jar协议、zip协议、wsjar协议、vfszip协议、war协议)包中找寻相应的所有class文件
	 ...
	 */
	//示例:入参->classpath*:test/cc*/spring-*.xml, 返回->classpath*:test/
	//示例:入参->classpath*:test/aa/spring-*.xml, 返回->classpath*:test/aa/
	...
	protected void doRetrieveMatchingFiles(String fullPattern, File dir, Set<File> result) throws IOException {
		...
		for (File content : dirContents) {
			...
			//查找到的子文件仍是目录且以根目录为开头(matchStart以先简单的判断是否需要递归进去，以提升性能)
